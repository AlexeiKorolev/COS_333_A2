...
class ChildThread (threading.Thread):
    def __init__(self, sock):
        ...
        self._sock = sock
        """
    Returns a course overviews query from the database 
    given a search substring
    for the department, course number, distribution area,
      and/or class title.
    If the query was successful, then the return tuple is
    (True, QUERY_RESULT),
    and if the query failed, then the return looks like
    (False, ERROR_MESSAGE)
    """
    def return_overviews_query(self, department='%', course_number='%',
    ...
    def details_format(self, class_info, course_info, crosslistings_info, res4):
    ...
    def run(self):
        ...
        # Is "with self._sock necessary here?"
        with self._sock:
            ...
            if actual_call[0] == 'get_overviews':
                ...
                payload =  self.return_overviews_query(department=args["dept"], 
                ...
                classinfo = self.get_class_info(class_id)
                ...
                if not classinfo[0]:
                    ...
                    infosets = self.get_course_info(classinfo[1][0])
                    ...
                    if not infosets[0]:
                        ...
                        # CHANGE THIS LATER
                        payload = [True, self.details_format(classinfo[1], infosets[1], infosets[2], infosets[3])]
